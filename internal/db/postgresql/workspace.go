...
func (h *hatchCatalogDb) CreateWorkspace(ctx context.Context, workspace *models.Workspace) apperrors.Error {
...
func (h *hatchCatalogDb) CreateWorkspaceWithoutStoredProcedure(ctx context.Context, workspace *models.Workspace) (err apperrors.Error) {
	tenantID := common.TenantIdFromContext(ctx)
	if tenantID == "" {
		return dberror.ErrMissingTenantID
	}
	...
	for _, d := range dirs {
		...
		if err := h.createSchemaDirectoryWithTransaction(ctx, d.objType, &dir, tx); err != nil {
			return err
		}
...
func (h *hatchCatalogDb) GetWorkspaceByLabel(ctx context.Context, variantID uuid.UUID, label string) (*models.Workspace, apperrors.Error) {
	tenantID := common.TenantIdFromContext(ctx)
	if tenantID == "" {
		return nil, dberror.ErrMissingTenantID
	}
...
func (h *hatchCatalogDb) GetCatalogForWorkspace(ctx context.Context, workspaceID uuid.UUID) (models.Catalog, apperrors.Error) {
...
func (h *hatchCatalogDb) CommitWorkspace(ctx context.Context, workspace *models.Workspace) apperrors.Error {
	tenantID := common.TenantIdFromContext(ctx)
	if tenantID == "" {
		return dberror.ErrMissingTenantID
	}
