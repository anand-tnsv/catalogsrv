...
func (h *hatchCatalogDb) CreateVersion(ctx context.Context, version *models.Version) error {
	// Retrieve tenantID and projectID from the context
	...
	query := `
		INSERT INTO versions (label, description, info, variant_id, catalog_id, project_id, tenant_id)
		VALUES ($1, $2, $3, $4, $5, $6, $7)
		RETURNING version_num;
	`

	// Execute the query and scan the returned version_num
	row := h.conn().QueryRowContext(ctx, query, label, version.Description, version.Info, version.VariantID, version.CatalogID, version.ProjectID, version.TenantID)
	...
	if err != nil {
		...
		if pgErr, ok := err.(*pgconn.PgError); ok {
			if pgErr.Code == "23505" && pgErr.ConstraintName == "unique_label_variant_catalog" { // Unique constraint violation
...
func (h *hatchCatalogDb) GetVersion(ctx context.Context, versionNum int, variantID, catalogID uuid.UUID) (*models.Version, error) {
	...
	query := `
		SELECT version_num, label, description, info, variant_id, catalog_id, project_id, tenant_id
		FROM versions
		WHERE version_num = $1 AND variant_id = $2 AND catalog_id = $3 AND tenant_id = $4 AND project_id = $5;
	`

	row := h.conn().QueryRowContext(ctx, query, versionNum, variantID, catalogID, tenantID, projectID)
	version := &models.Version{}
	err = row.Scan(&version.VersionNum, &version.Label, &version.Description, &version.Info, &version.VariantID, &version.CatalogID, &version.ProjectID, &version.TenantID)
	if err != nil {
		if err == sql.ErrNoRows {
...
func (h *hatchCatalogDb) SetVersionLabel(ctx context.Context, versionNum int, variantID, catalogID uuid.UUID, newLabel string) error {
	...
	query := `
		UPDATE versions
		SET label = $1
		WHERE version_num = $2 AND variant_id = $3 AND catalog_id = $4 AND tenant_id = $5 AND project_id = $6
		RETURNING version_num;
	`

	row := h.conn().QueryRowContext(ctx, query, newLabel, versionNum, variantID, catalogID, tenantID, projectID)
...
func (h *hatchCatalogDb) UpdateVersionDescription(ctx context.Context, versionNum int, variantID, catalogID uuid.UUID, newDescription string) error {
	...
	row := h.conn().QueryRowContext(ctx, query, newDescription, versionNum, variantID, catalogID, tenantID, projectID)
	var returnedVersionNum int
	err = row.Scan(&returnedVersionNum)
	if err != nil {
		if err == sql.ErrNoRows {
...
func (h *hatchCatalogDb) DeleteVersion(ctx context.Context, versionNum int, variantID, catalogID uuid.UUID) error {
	...
	result, err := h.conn().ExecContext(ctx, query, versionNum, variantID, catalogID, tenantID, projectID)
	if err != nil {
...
func (h *hatchCatalogDb) CountVersionsInCatalogAndVariant(ctx context.Context, catalogID, variantID uuid.UUID) (int, error) {
	...
	query := `
		SELECT COUNT(*)
		FROM versions
		WHERE catalog_id = $1 AND variant_id = $2 AND tenant_id = $3 AND project_id = $4;
	`

	var count int
	err = h.conn().QueryRowContext(ctx, query, catalogID, variantID, tenantID, projectID).Scan(&count)
	if err != nil {
...
func (h *hatchCatalogDb) GetNamedVersions(ctx context.Context, catalogID, variantID uuid.UUID) ([]models.Version, error) {
	...
	query := `
		...
		FROM versions
		WHERE catalog_id = $1 AND variant_id = $2 AND tenant_id = $3 AND project_id = $4 AND label IS NOT NULL;
	`

	rows, err := h.conn().QueryContext(ctx, query, catalogID, variantID, tenantID, projectID)
	if err != nil {
...
func (h *hatchCatalogDb) GetVersionByLabel(ctx context.Context, label string, catalogID, variantID uuid.UUID) (*models.Version, error) {
	...
	query := `
		SELECT version_num, label, description, info, variant_id, catalog_id, project_id, tenant_id
		FROM versions
		WHERE label = $1 AND catalog_id = $2 AND variant_id = $3 AND tenant_id = $4 AND project_id = $5;
	`

	row := h.conn().QueryRowContext(ctx, query, label, catalogID, variantID, tenantID, projectID)
	version := &models.Version{}
	err = row.Scan(&version.VersionNum, &version.Label, &version.Description, &version.Info, &version.VariantID, &version.CatalogID, &version.ProjectID, &version.TenantID)
	if err != nil {
		if err == sql.ErrNoRows {
